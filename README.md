...
- **官方数据增补**：`scripts/pull_usfs.py` 拉取 [USFS National Forest System Trails](https://data.fs.usda.gov/geodata/)（美国林务局，**公有领域**）全国步道几何 + 官方编号，`scripts/augment_official.py` 按名字对齐到步道、长度吻合时以官方高精度线形覆盖 OSM 结果，标为最高档 `confidence=official`。约 4.3k 条国家森林步道由此获得官方级路线。
- **路线置信分级**：其余路线按「长度完整度 + 同名步道占比 + 官方/实地来源标签（`operator`/`ref`/`sac_scale`）+ 通行权（剔除 `access=private`、`informal=yes`）」定级为 high/medium/low，写入 `confidence` 列。App 里 **official/high 显示为实线（可放心跟随）、medium 为虚线参考、low 视为"暂无收录路线"**——宁可不显示也不误导。全量 77,591 条中约 4.3k 官方 / 13.8k 高可信 / 7.0k 参考。**运行时不联网**，详情页小地图与录制页直接读库绘制橙色路线，录制页相机自动适配整条路线；OSM 未覆盖的步道提示「暂无收录路线」。
- **收藏**：收藏列表。
